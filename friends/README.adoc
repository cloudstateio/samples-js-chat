...
We use https://docs.docker.com/compose/#compose-documentation[docker-compose] to run these docker images together. Before starting, you must edit the `docker-compose.yaml` to replace the prebuilt `cloudstateio/samples-js-chat-friends:latest` by your own that you had published in  the <<building-the-friends-service,Building the Friends service>> step.
...
image::../docs/DockerCompose_Friends_UserFunction.png[docker-compose, Friends "user-function"]


=== Testing with gRPCurl

With a new terminal session, we can use https://github.com/fullstorydev/grpcurl[gRPCurl] to test our service. Make sure you have installed gRPCurl as mentioned in the <<../README.adoc#js-devenv-setup-for-cloudstate,JavaScript Dev environment setup for Cloudstate>> section.
